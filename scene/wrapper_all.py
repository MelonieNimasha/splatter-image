...
class WrapperDatasetAll(Dataset):
    ...
    def modify_output(self, original_item):
        noised_images = original_item["noised_images"]
        noised_cameras = original_item["noised_cameras"]
        all_cameras = original_item["all_cameras"]

        if self.perm == "first":
            permuted_indices = torch.tensor([0,1,2,3,4,5])

        elif self.perm == "middle":
            middle_index = 3
            permuted_indices = torch.cat((torch.arange(middle_index, noised_images.size(0)), torch.arange(middle_index)), dim=0)

        else:
            permuted_indices = torch.randperm(noised_cameras["world_view_transform"].size(0))
        

        fovy = torch.concat([noised_cameras["FoVy"][permuted_indices] , all_cameras["FoVy"]])
        fovx = torch.concat([noised_cameras["FoVx"][permuted_indices] , all_cameras["FoVx"]])
        full_proj_transform = torch.concat([noised_cameras["full_proj_transform"][permuted_indices] , all_cameras["full_proj_transform"]])
        world_view_transform = torch.concat([noised_cameras["world_view_transform"][permuted_indices] , all_cameras["world_view_transform"]])
        camera_center = torch.concat([noised_cameras["camera_center"][permuted_indices] , all_cameras["camera_center"]])
        ...
        focals_pixels = torch.stack([fovy_pixels, fovx_pixels], dim=1) 
        ...
        images_and_camera_poses = {
            ...
            "view_to_world_transforms": view_to_world_transform,
